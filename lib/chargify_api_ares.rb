...
module Chargify
  ...
  class ProductFamily < Base
    ...
    def find_coupon_by_code(code)
      Coupon.find_by_product_family_id_and_code(id, code)
    end
  end
  ...
  class Coupon < Base
    ...
    def self.find_by_product_family_id_and_code(product_family_id, code)
      coupon = nil
      connection.get("/product_families/#{product_family_id}/coupons/find.#{self.format.extension}?code=#{code}", self.headers).tap do |response|
        coupon = new response
      end
      coupon
    end
