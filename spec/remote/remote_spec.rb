...
describe "Remote", :remote => false do
  before(:all) do
    clear_site_data
    setup_plans
    setup_customer
  ...
  describe "creating a new subscription to a product with a trial" do
    context "when providing valid attributes for the customer and the payment profile" do
      ...
      before(:each) do
        @subscription = create_once(:subscription, group_id) do
      ...
      it "sets the current_period_started_at attribute to now" do
        @subscription.current_period_started_at.utc.should be_within(300).of(now.utc)
      end
      
      it "sets the current_period_ends_at attribute to 1 month from now" do
        @subscription.current_period_ends_at.utc.should be_within(300).of(one_month_from_now.utc)
      end
  ...
  describe "importing a subscription to a product with a trial and a next billing date 10 days from now" do
    context "when giving valid attributes for the customer and the payment profile" do
      ...
      it "sets the current_period_ends_at attribute to 1 month from now" do
        @subscription.current_period_ends_at.utc.should be_within(300).of(ten_days_from_now.utc)
      end
  ...
  def create_once(type, unique_id, &block)
    hierarchy = unique_id.to_s
